...
import { deleteBook, exportBooks, getAllBooks } from "../../../http";
...
const ManageBook = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (data?.books) {
        const avlblBooks = data.books.filter(
          (book) => book?.status == "Available"
        );
        setAvailableBooks(avlblBooks.length);
      }
      console.log(data);
      setData(data);
    } catch (error) {
  ...
  return (
    <div className="manage__section bg">
      ...
      <div>
        <h1 style={{ marginLeft: 15 }}>Available books : {availableBooks}</h1>
      </div>
